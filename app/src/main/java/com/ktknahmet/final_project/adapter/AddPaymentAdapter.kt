...
class AddPaymentAdapter (private val alldata:List<AddPayment>): RecyclerView.Adapter<AddPaymentAdapter.DetailsHolder>() {
    ...
    override fun onBindViewHolder(holder: DetailsHolder, position: Int) {
        //oluşan verilerin positionuna göre döndürme işlemi
        val dateNow = Date()
        //günün tarihini alırız
        val sdf2 = SimpleDateFormat("dd/MM/yyyy")
        //tarihi formatlarız
        val selectDateTime = sdf2.format(dateNow)
        val date = sdf2.parse(selectDateTime)
        val selectDate = date!!.time
        //formatlanan tarihin long tipinden time'ını yani süresini alırız
        ...
        with(holder) {
            ...
            //çıkan tarihi sonucunu bölerek güne çevirmiş olduk

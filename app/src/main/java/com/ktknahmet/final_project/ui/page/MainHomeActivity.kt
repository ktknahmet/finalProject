...
class MainHomeActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setUI(myTheme, this)
         //Base activity'de setUI temayı ayarlarız  ve burada çağırırız
        val navHostFragment = supportFragmentManager.findFragmentById(R.id.navHostFragment) as NavHostFragment?
        val navController = navHostFragment!!.navController

        //navController sayesinde navigation nav Graph oluştururuz ve bottom navigation barı aktif hale getiririz
        binding.bottomNavbarMain.setupWithNavController(navController)
        navController.addOnDestinationChangedListener { _, destination, _ ->
            when (destination.id) {
                //oluşan sayfalara göre bottomNavbarı gizleriz veya visible yaparız
